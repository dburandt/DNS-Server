...
public class DNSResponse {
	...
	void dumpStandardResponse() {
		System.out.println("\n    " + DNSlookup.fqdn + " " + 
		answerList[0].getTtl() + " " + answerList[0].getIPaddress().toString().substring(1));
	}

	void dumpTraceResponse() {
		...
		if ((answerCount > 0) && answerList[0].getCNAME() == null) {
			System.out.println(DNSlookup.fqdn + " " + answerList[0].getTtl() + 
					" " + answerList[0].getIPaddress().toString().substring(1));
		}
	...
	public DNSResponse (byte[] data, int len, DNSQuery q, Boolean tracingOn) {
		...
		// check if query id from request & response match
		if (queryID != q.getQueryID()) {
			if (comparisonCount == 30) {
				...
				comparisonCount++;
				try {
					DNSlookup.sendQuery(query.getQueryAsString(), query.getServerAddress());
				} catch (IOException e) {
		...
		if (replyCode == 3 && tracingOn) {
			System.out.println(DNSlookup.fqdn + " -1 0.0.0.0");
			return;
		} else if (replyCode == 3 && !tracingOn) {
			System.out.println("\n    " + DNSlookup.fqdn + " -1 0.0.0.0");
		} else if (replyCode == 5) {
			System.out.println(DNSlookup.fqdn + " -4 0.0.0.0");
		}
		...
		if (tracingOn) {
			...
			if (answerList[0].getCNAME() == null) { dumpStandardResponse(); }
		}
		
		// if authoritative but a cname
		if (authoritative && answerCount == 0) {
			System.out.println(DNSlookup.fqdn + " -4 0.0.0.0");
		} else if (authoritative && (answerList[0].getCNAME() != null)) {
			try {
				DNSlookup.sendQuery(getCNAME(), DNSlookup.rootNameServer);
			...
			try {
				DNSlookup.sendQuery(q.getQueryAsString(), InetAddress.getByName(nsList[0].getCNAME()));
			} catch (UnknownHostException e) {
				System.out.println(DNSlookup.fqdn + " -4 0.0.0.0");
			} catch (IOException e) {
				System.out.println(DNSlookup.fqdn + " -4 0.0.0.0");
	...
	public String getAaFQDN() {
		return aaFQDN;
	}

	public void setAaFQDN(String aaFQDN) {
		this.aaFQDN = aaFQDN;
	}
